...
!! elevation bands
     ...
     &                                tavband(ib,j) * sub_timp(ib,isub)
          ...
          !! Modified to Yang's Thesis accordingly.
          sub_sftmp(ib,isub) = 0.0006 * welev(isub) + 0.1245 * acculat(isub) - 0.2382 * acculon(isub) + 25.722
...
!! no elevation bands
	...
	ib = 1
     ...
     &       (sub_smfmx(ib,isub) - sub_smfmn(ib,isub)) / 2.    !! 365/2pi = 58.09
      ...
      !!write(810, *) 'welev(', isub, ') = ', welev(isub)
      !!close(810)
      return
      end